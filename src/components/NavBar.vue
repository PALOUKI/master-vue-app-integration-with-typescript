            ...
            <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div class="inline-flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                    ...
                    <div v-if="authStore.isAuthenticated" class="flex items-center gap-4">
                        <router-link :to="{ name: routes.cart }"
                            :class="[String($route.name).endsWith('cart') ? 'md:bg-blue-50' : 'bg-gray-50']"
                            class="relative group p-2 rounded-full hover:bg-blue-50 transition-all">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" class="group-hover:text-blue-600" 
                                :class="[String($route.name).endsWith('cart') ? 'md:text-blue-600' : 'text-gray-700']">
                                <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z" />
                                <path d="M3 6h18" />
                                <path d="M16 10a4 4 0 0 1-8 0" />
                            </svg>

                            <span v-if="cartStore.currentCart?.totalQuantity"
                                ...
                                {{ cartStore.currentCart.totalQuantity }}
                            </span>
                        </router-link>
                    </div>
                ...
                <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
                    <ul
                        ...
                        <li>
                            <RouterLink to="/"
                                :class="[String($route.name).endsWith('home') ? 'md:text-blue-500' : 'text-gray-800']"
                                class="block py-2 px-3 bg-brand rounded  md:bg-transparent md:p-0">Home</RouterLink>
                        </li>
                        <li>
                            <RouterLink to="/products"
                                :class="[String($route.name).endsWith('products') ? 'md:text-blue-500' : 'text-gray-800']"
                        ...
                        <li>
                            <a href="#"
                                class="block py-2 px-3 text-heading rounded hover:bg-neutral-tertiary md:hover:bg-transparent md:border-0 md:hover:text-fg-brand md:p-0 md:dark:hover:bg-transparent">Recipes</a>
                        </li>
                        <li>
                            <a href="#"
